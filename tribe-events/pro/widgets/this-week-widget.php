...
<div class="tribe-this-week-widget-wrapper tribe-this-week-widget-<?php echo esc_attr( $this_week_template_vars['layout'] ); ?> <?php echo esc_attr( tribe_this_week_widget_class( $this_week_query_vars['tax_query'] ) ); ?>" <?php echo apply_filters( 'tribe_events_this_week_header_attributes', $this_week_data_attrs ); ?> >
	...
	<?php do_action( 'tribe_events_before_this_week_title' ) ?>
		<?php $thismonday = tribe_get_this_week_first_week_day( $this_week_template_vars['start_date'] );
			$thismonday = date_create( $thismonday )->format( 'j' ); ?>
		<?php $thissunday = tribe_get_this_week_first_week_day( $this_week_template_vars['end_date'] );
			$thissunday = date_create( $thissunday )->format( 'j' ); ?>
		<?php $thismonth = tribe_get_this_week_first_week_day( $this_week_template_vars['end_date'] );
			$thismonth = date_create( $thismonth)->format( 'F' ); ?>
		<h2 class="tribe-events-page-title"><?php echo $thismonth ?> <?php echo $thismonday; ?>-<?php echo $thissunday; ?></h2>
	...
	<div class="tribe-this-week-widget-weekday-wrapper <?php echo esc_html( 'true' === $this_week_template_vars['hide_weekends'] ? 'tribe-this-week-widget-hide-weekends' : '' );  ?>" >
		...
		<?php foreach ( $week_days as $day ) : ?>
			<?php 
				if (!empty($day['has_events'])) {
					tribe_get_template_part( 
						'pro/widgets/this-week/loop-grid-day', 
						'grid-dau', 
						array( 
							'day' => $day, 
							'this_week_template_vars' => $this_week_template_vars 
						) 
					); 				
				}
			?>
